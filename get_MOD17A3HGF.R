...
# MCD43A2
setwd('/projectnb/modislc/users/twgreen/MODISProject/MOD17A3HGF/')
if(year==2000){
  mod_dates <- seq(1,365)
  for(i in 55:365){
    d = mod_dates[i]
    ...
    url <- paste('https://e4ftl01.cr.usgs.gov/MOLT/MOD17A3HGF.061/',year,'.',mm,'.',dd,'/',sep='')
    
    system(paste('wget --user=twgreen --password=BUremotesensing1! -l1 -r --no-parent -A "MOD17A3HGF.A',year,d_str,'.',tile,'.061.*.hdf" ',url,sep=''))
