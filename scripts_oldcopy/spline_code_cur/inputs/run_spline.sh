...
if [ -z "$1" ]
then
	echo Usage \"./run_spline.sh in_tile
	exit 1
else
	in_tile=$1
fi
...
out_dir="/projectnb/modislc/users/joshgray/newSplines/testout"
## name for the outputs will start with this string
out_stem="c6_str5_halfyear_link"
...
nbar_dir="/projectnb/modislc/users/joshgray/newSplines/mcd43a4_link"
# qa_dir="/projectnb/modislc/data/mcd12_in/c6/mcd43a2"
qa_dir="/projectnb/modislc/users/joshgray/newSplines/mcd43a2_link"
first_yr=$(( start_yr + 1 ))
...
#	echo "No outputs for tile $in_tile. Will process"
	...
	echo ../bin/modis_smooth_t --tile_id $in_tile --chunk $chunk_size --smooth 1,2,4,5,6,7,8 --prefilter 8 --residual 8 --quantile $quant_flag --start_year $start_yr --num_yrs $num_yrs --stride $stride --out_stem $out_stem --out_dir $out_dir --nbar_dir $nbar_dir --qa_dir $qa_dir --extract_path $extract_path
	...
	../bin/modis_smooth_t --tile_id $in_tile --chunk $chunk_size --smooth 1,2,4,5,6,7,8 --prefilter 8 --residual 8 --quantile $quant_flag --start_year $start_yr --num_yrs $num_yrs --stride $stride --out_stem $out_stem --out_dir $out_dir --nbar_dir $nbar_dir  --qa_dir $qa_dir --extract_path $extract_path
#fi 
