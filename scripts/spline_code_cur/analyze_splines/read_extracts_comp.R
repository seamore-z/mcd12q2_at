...
if(debug==0) {
  ...
  in_dir = "../outputs_test2"
  in_stem = "c6_str5"
  tile= "h12v04"
  ## should be actual year - 1
  start_year = 2000
  num_years = 5
}
...
for(y in 1:num_years) {
    in_files[y] = paste(in_dir,"/",in_stem,".",tile,".",(start_year+y-1),"-",(start_year+y+1),".extract.txt",sep="")
 #     print(in_files[[s]][y])
}
...
b_names = c("Red","NIR","SWIR","EVI2","flag")
  ...
  for(i in 1:num_site_ids) {
    ...
    for(n in 1:num_dates) {
      for(j in 1:length(temp_ind)) {
        if(temp_ind[j]) {
          snow_flags[n]=max(snow_flags[n],all_post[[j]][nbands,n],na.rm=T)
        }
      }
    }
    cols <- mycols[snow_flags + 1]
