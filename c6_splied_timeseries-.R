...
splined <- matrix(NA,(365*3),length(sam))
year <- 2020 #Needs to be the year of interest
# BU all years
for(i in 1:(365*3)){
  if(i==1){
    data <- readBin(paste('/projectnb/modislc/projects/sat/data/spline/',tile,'/tg_v1.',tile,'.',(year-1),'.evi2.bip',sep=''),
    ...
    data <- readBin(paste('/projectnb/modislc/projects/sat/data/spline/',tile,'/tg_v1.',tile,'.',(year-0),'.evi2.bip',sep=''),
                    what="integer",n=cnt,size=2,endian="little")
  ...
  splined[i,length(as.matrix(rast)[sam[num]])] <- as.matrix(rast)[sam[num]]
  if(i%%100==0) print(i)
}

splinedf <- as.data.frame(splined)
